...
, '["a", "b"]'::jsonb ?& array['a', 'b'], '["a", "b"]'::jsonb || '["c", "d"]'::jsonb, '{"a": "b"}'::jsonb - 'a', '["a", "b"]'::jsonb - 1, '["a", {"b":1}]'::jsonb #- '{1,b}'`,
    ...
    {
      title: 'window function with parentses',
      sql: [
        `SELECT sum(salary) OVER (w), avg(salary) OVER (w)
        FROM empsalary
        WINDOW w AS (PARTITION BY depname ORDER BY salary DESC);`,
    ...
    {
      title: 'create table with distributed by',
      sql: [
        `create table posts(id serial primary key, name varchar(128)) distributed by (id)`,
        `CREATE TABLE "posts" (id SERIAL PRIMARY KEY, name VARCHAR(128)) distributed by (id)`
      ]
    },
  ]
  ...
  describe('pg ast', () => {
    ...
    it('should has loc property', () => {
      ...
      const opt = { database: 'greenplum', parseOptions: { includeLocations: true } }
      const ast = parser.astify(sql, opt)
